...
def main():
    ...
    selected_color = st.selectbox(
        "Selecione um nível de qualidade", list(colors.keys())
    )
    ...
    selected_instrument = st.selectbox(
        "Selecione um instrumento", scientific_instruments
    )
    ...
    if st.button("Submeter"):
        st.session_state.table_data.append(
            (d, selected_instrument, selected_Campaign, selected_extra, selected_value)
        )
    ...
    if st.session_state.table_data:
        df = pd.DataFrame(
            ...
            columns=["Date", "Instrument", "Campaign", "Project", "Data Quality"],
        )
